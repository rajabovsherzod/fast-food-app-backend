...
export const getAssignedOrders = async (deliveryId) => {
    return await db
        .select()
        .from(orders)
        .where(and(eq(orders.deliveryId, deliveryId), sql `${orders.status} IN ('ready', 'delivering')`))
        .orderBy(desc(orders.createdAt));
};
export const getOrderDetails = async (orderId, deliveryId) => {
...
export const completeDelivery = async (orderId, deliveryId) => {
    const [order] = await db
        .select()
        .from(orders)
        .where(and(eq(orders.id, orderId), eq(orders.deliveryId, deliveryId)));
...
export const updateLocation = async (deliveryId, orderId, latitude, longitude) => {
...
export const getStatistics = async (deliveryId) => {
    const today = new Date();
    today.setHours(0, 0, 0, 0);
    const [stats] = await db
        .select({
        totalDeliveries: sql `count(*)::int`,
        todayDeliveries: sql `count(*) FILTER (WHERE ${orders.deliveredAt} >= ${today})::int`,
        activeDeliveries: sql `count(*) FILTER (WHERE ${orders.status} = 'delivering')::int`,
        todayEarnings: sql `sum(CASE WHEN ${orders.deliveredAt} >= ${today} THEN ${orders.deliveryPrice}::numeric ELSE 0 END)`,
        totalEarnings: sql `sum(${orders.deliveryPrice}::numeric)`,
    })
        .from(orders)
        .where(and(eq(orders.deliveryId, deliveryId), eq(orders.status, "delivered")));
    return stats;
};
export const getDeliveryHistory = async (deliveryId, page = 1, limit = 20) => {
